<script>
    ...
    export let isMattressBooking = false
    ...
    onMount(async () => {
        ...
        if (booking !== undefined) {
            ...
            // formValid = $isValid
            setTimeout(() => formValid = $isValid, 0)
            if (booking !== null && !isMattressBooking)
                isMattressBooking = booking?.mattress_booking
        }
    ...
    function updateBooking() {
        if (booking === null)
            booking = {}
        Object.entries($form).forEach(e => booking[e[0]] = e[1])
        if (booking.paid && booking.date_paid === '')
            booking.date_paid = DateTime.now().toSQLDate()

        booking.membership_count = isMattressBooking ? 0 : booking.membership_needed ? booking.people_count : 0
        ...
        if (events.length > 0) {
            events = events.map(e => {
                ...
                return {
                    ...
                    confirmed: existingBookingIndex > -1 ? existingBookingConfirmed : (e.max_participants === null || ((e.bookings === null || e.bookings.length === 0) && Number($form.people_count) <= e.max_participants)
...
<form id="bookingForm" class="uk-form" class:uk-form-horizontal={!$screenWidthL}>
        ...
        <div class="uk-form-controls uk-grid-small uk-margin-remove-left@l" id="form-personals" uk-grid>
            <div class="uk-width-1-1 uk-inline">
                <input class="uk-input" id="form-name" name="name" type="text" placeholder="Name"
                       ...
                       on:keyup={formUpdate} on:blur={formUpdate} bind:value={$form.name}>
                ...
                {#if booking?.mattress_booking}
                    <span class="uk-label uk-label-warning uk-position-center-right uk-margin-small-right">Mattress</span>
                {/if}
            </div>
            <div class="uk-width-1-2@s">
                <input class="uk-input" id="form-email-no1pw-search" name="email" type="email" placeholder="Email" autocomplete="'off"
                       ...
                       on:keyup={formUpdate} on:blur={formUpdate} bind:value={$form.email}>
            </div>
            <div class="uk-width-1-2@s">
                <input class="uk-input" id="form-city" name="city" type="text" placeholder="City"
                       ...
                       on:keyup={formUpdate} on:blur={formUpdate} bind:value={$form.city}>
            </div>
        </div>
    </div>
    <div class="uk-margin-medium">
        ...
        <div class="uk-form-controls uk-grid-small uk-margin-remove-left@l" id="form-dates" uk-grid>
            <div class="uk-width-1-2@s">
                <input class="uk-input" id="form-arrival" name="arrival" type="text" placeholder="Arrival" readonly
                       ...
                       on:change={formUpdate} on:blur={formUpdate} bind:value={$form.arrival}>
            </div>
            <div class="uk-width-1-2@s">
                <input class="uk-input" id="form-departure" name="departure" type="text" placeholder="Departure" readonly
                       ...
                       on:change={formUpdate} on:blur={formUpdate} bind:value={$form.departure}>
            </div>
        </div>
    </div>
    <div class="uk-margin-medium">
        ...
        <div class="uk-form-controls uk-grid-small uk-margin-remove-left@l" id="form-accounting" uk-grid>
            <div class="uk-width-1-2@s uk-text-nowrap">
                <input class="uk-input uk-margin-small-right uk-form-width-small" name="people_count" type="number"
                       ...
                       on:change={formUpdate} on:blur={formUpdate} bind:value={$form.people_count}> People count
            </div>
            {#if !isMattressBooking}
                ...
                <div class="uk-width-1-2@s uk-text-nowrap">
                    <input class="uk-input uk-margin-small-right uk-form-width-small" name="stay_rate" type="number"
                           ...
                           on:change={formUpdate} on:blur={formUpdate} bind:value={$form.stay_rate}> Stay rate
                </div>
                <div class="uk-width-1-2@s uk-text-nowrap">
                    <input class="uk-checkbox" id="form-institutional" name="institutional" type="checkbox"
                           on:change={formUpdate} on:blur={formUpdate} bind:checked={$form.institutional}>&nbsp; Institutional rate
                ...
                <div class="uk-width-1-2@s">
                    <div>
                        <input class="uk-checkbox uk-margin-small-right" name="early_pay" type="checkbox"
                               on:change={formUpdate} on:blur={formUpdate} bind:checked={$form.early_pay}>
                    ...
                    {#if booking !== null}
                        <div>
                            {#if booking?.paid}
                                <input class="uk-checkbox uk-margin-small-right" id="form-paid" name="paid" type="checkbox"
                                       on:change={formUpdate} on:blur={formUpdate} bind:checked={$form.paid}> Paid
                            {:else}
                        ...
                        <div>
                            <input class="uk-checkbox uk-margin-small-right" name="cancelled" type="checkbox"
                                   on:change={formUpdate} on:blur={formUpdate} bind:checked={$form.cancelled}> Cancelled
                        </div>
                    {/if}
                    <div>
                        <input class="uk-checkbox uk-margin-small-right" name="membership_needed" type="checkbox"
                               on:change={formUpdate} on:blur={formUpdate} bind:checked={$form.membership_needed}> Membership/s needed
                    </div>
                </div>
                <div class="uk-width-1-2@s">
                    <div>
                        <input class="uk-checkbox uk-margin-small-right" id="form-longstayer" name="longstayer" type="checkbox"
                               on:change={formUpdate} on:blur={formUpdate} bind:checked={$form.longstayer}> Longstayer
                    </div>
                    <div>
                        <input class="uk-checkbox uk-margin-small-right" id="form-no_stay_pay" name="no_stay_pay" type="checkbox"
                               on:change={formUpdate} on:blur={formUpdate} bind:checked={$form.no_stay_pay}> Doesn't pay for stay
                    </div>
                    <div>
                        <input class="uk-checkbox uk-margin-small-right" id="form-no_meal_pay" name="no_meal_pay" type="checkbox"
                               on:change={formUpdate} on:blur={formUpdate} bind:checked={$form.no_meal_pay}> Doesn't pay for meals
                    </div>
                </div>
            {:else}
                <div class="uk-width-1-2@s">
        ...
        <div class="uk-form-controls uk-grid-small uk-margin-remove-left@l" id="form-info" uk-grid>
            <div class="uk-width-1-1">
                <textarea class="uk-textarea" name="info"
                          ...
                          on:keyup={formUpdate} on:blur={formUpdate} bind:value={$form.info}></textarea>
            </div>
        </div>
    </div>
    <div class="uk-margin-medium">
        ...
        <div class="uk-form-controls uk-grid-small uk-margin-remove-left@l" id="form-group_name" uk-grid>
            <div class="uk-width-1-2@s">
                <input class="uk-input" name="group_name" type="text" placeholder="Group name"
                       ...
                       on:keyup={formUpdate} on:blur={formUpdate} bind:value={$form.group_name}>
            </div>
        </div>
    </div>
    <div class="uk-margin-medium">
        ...
        <div class="uk-form-controls uk-grid-small uk-margin-remove-left@l" id="form-website" uk-grid>
            <div class="uk-width-1-1">
                <input class="uk-checkbox" id="form-show_on_website" name="show_on_website" type="checkbox"
                       on:change={formUpdate} on:blur={formUpdate} bind:checked={$form.show_on_website}> Show on website
            </div>
        </div>
    </div>
</form>